...
unsigned acq_buffer_full2(unsigned cidx)
{
       ...
       ch->acq.doppler_idx++)
  {
    ...
    {
      printf("coarse acq best doppler: %f\n", max_doppler);
      ...
      ch->acq.ratio = tempd;
      retval = SUCCESS;
      printf("freq: %f, doppler = %f, detection ratio: %f \n",
...
void init_fft_acq()
{
  ...
  //kiss_fft_cpx *fft_buf;
        ...
        printf("precalculating C/A code FFT tables...");
	sample_idx = 0;
  ...
  else
  {
  		...
  		if(tempc == '/')
      {
        ...
        if(tempc == 'A')
        {
          ...
          printf("Allocating PRN %d to channel %d\n",
