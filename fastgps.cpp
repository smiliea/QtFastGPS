...
int MainWindow::run_fastgps()
{
  ...
  if (retval == PROBLEM)
  {
    ...
    printf("ahhhh couldn't read config file.\nCheck that "
  ...
  {
    sprintf(Buffer, "Using sp3 file for navigation information.\n");
    ui->textBrowser->append(Buffer);
  ...
  if (!data_file)
  {
    sprintf(Buffer, "ahhhh couldn't open data file.\nCheck that path to data "
  ...
  if(system_vars.acq_log_flag == DEBUG_LOGGING)
    printf("Generating Acquisition Debug Information.\n");
  if(system_vars.nav_log_flag == DEBUG_LOGGING)
    printf("Generating Navigation Debug Information.\n");
  ...
  while (!feof(data_file) && (system_vars.process_time < system_vars.run_time))
  {
      ...
      if(system_vars.file_acquisition == ON)
      {
        ...
        if (acq_buf_write_pos >= system_vars.acq_buf_len)
        {
          ...
          if(system_vars.num_channels > 0)
          {
            ...
            printf("Satellites acquired using input file.\n"
                           "Starting tracking.\n");
          }
          else
          {
            system_vars.Rx_State = LOST;     // we have found nothing, exit
            printf("No satellites acquired using input file.\n");
          }
        }
      }
      else
      {
        ...
        if (acq_buf_write_pos >= system_vars.acq_buf_len)
        {
          ...
          for (j = 0; j < system_vars.num_search_svs; j++)
          {
            ...
            if(retval == SUCCESS)
            {
              printf("Found PRN %d, allocated to channel %d\n",
                             c[system_vars.search_channel].prn_num,
    ...
    if(system_vars.Rx_State >= TRACKING)
    {
          ...
          && (system_vars.position_status == 0))
      {
          ...
          printf("Performing Doppler Position Calculation.\n");
          ...
          && (system_vars.snap_shot_flag == YES))
      {
          ...
          if(system_vars.Rx_State != NAV)
          {
            ...
            printf("Starting Single Epoch Position Calculation.\n");
          }

          printf("Performing Single Epoch Position Calculation.\n");
          system_vars.next_pvt_time = system_vars.process_time + system_vars.PVT_INTERVAL;
         ...
         (system_vars.snap_shot_flag == NO))
      {
          ...
          {
            system_vars.Rx_State = NAV;
            printf("Starting Least Squares Position Calculation.\n");
          }
  ...
  printf("finished. good night.\n");

  return 0;
}

int MainWindow::read_config_file()
{
  double testd = 0;
  int testi=0;
  char tempc=0;
...
void MainWindow::update_acq_log()
{
...
void MainWindow::update_tracking_log()
{
  if(system_vars.tracking_log_flag >= NORMAL_LOGGING)
  {
    if(system_vars.tracking_log == NULL)
...
void MainWindow::update_nav_log()
{
  unsigned chan;
